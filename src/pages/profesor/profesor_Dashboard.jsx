    ...
    // Datos base
    const [cursoAsig, setCursoAsig]       = useState([])   // lista de { cursos, asignaturas }
    ...
    // Curso seleccionado globalmente
    const [cursoId, setCursoId]           = useState('')
    const [asignaturaId, setAsignaturaId] = useState('')

    // Tab: Ingreso de notas
    const [evalSelId, setEvalSelId]       = useState('')
    const [notasEval, setNotasEval]       = useState([])   // [{ alumno_id, nombre, nota }]
    const [notasInput, setNotasInput]     = useState({})   // { alumno_id: valor }
    const [loadingNotas, setLoadingNotas] = useState(false)

    // Tab: Asistencia
    const [fechaAsist, setFechaAsist]     = useState(hoy)
    const [asistencia, setAsistencia]     = useState({})   // { alumno_id: estado }

    // Tab: Crear evaluación
    const [evalForm, setEvalForm] = useState({
        nombre: '', porcentaje: 20, fecha: hoy,
    })

    // Tab: Anotaciones
    const [anotForm, setAnotForm] = useState({
        alumnoId: '', tipo: 'positiva', descripcion: '',
    })

    // Tab: Retiros
    const [retiroForm, setRetiroForm] = useState({
        alumnoId: '', tipo: 'conducta', motivo: '',
    })
    ...
    // Evaluaciones del curso+asignatura seleccionados
    const evalsFiltradas = evaluaciones.filter(e =>
        (!cursoId      || e.cursos?.id === cursoId      || e.cursos?.nivel + e.cursos?.letra === cursoMap[cursoId]?.nivel + cursoMap[cursoId]?.letra) &&
        (!asignaturaId || e.asignaturas?.id === asignaturaId)
    )

    // ── Cambio de curso: recargar alumnos y alertas ───────────────────────────
    const handleCursoChange = async (id) => {
        ...
        const initAsist = {}
        listaAlumnos.forEach(a => { initAsist[a.id] = 'presente' })
        setAsistencia(initAsist)
    }
    ...
    const handleEvalChange = async (id) => {
        ...
        // Merge con lista de alumnos del curso
        const notasMap = {}
    ...
    const handleGuardarAsistencia = async () => {
        ...
        const registros = Object.entries(asistencia).map(([alumnoId, estado]) => ({ alumnoId, estado }))
    ...
    const handleCrearEval = async (e) => {
        ...
        return notify('error', 'Selecciona curso, asignatura y nombre de evaluación.')
        }
    ...
    return (
        ...
        <div className="grid-4 section">
            ...
            <div>
                <div className="stat-value">{cursoList.length}</div>
            ...
            <div>
                <div className="stat-value">{evaluaciones.length}</div>
            ...
            <div>
                ...
                <div className="stat-label">Alumnos en curso</div>
            </div>
            </div>
            <div className="stat-card">
            ...
            <div>
                ...
                <div className="stat-label">Alertas en curso</div>
            </div>
            </div>
        </div>
        ...
        <div className="card section">
            ...
            {alumnos.length > 0 && (
                <span className="badge default">{alumnos.length} alumnos</span>
            )}

            {alertas.length > 0 && (
                <span className="badge negativa">⚠️ {alertas.length} alertas</span>
            )}
            </div>
        </div>
                ...
                {!loadingNotas && evalSelId && notasEval.length > 0 && (
                    ...
                    <div className="table-wrap">
                        ...
                        <thead>
                            <tr>
                            <th>Alumno</th>
                            <th style={{ width: 140 }}>Nota (1.0 – 7.0)</th>
                            </tr>
                        </thead>
                        <tbody>
                            {notasEval.map(n => (
                            <tr key={n.alumno_id}>
                                ...
                                <input
                                    ...
                                    onChange={e =>
                                    setNotasInput(p => ({ ...p, [n.alumno_id]: e.target.value }))
                                    }
                                />
                                </td>
                            </tr>
                        ...
                        <tbody>
                            ...
                            <tr key={a.id}>
                                ...
                                <div style={{ display: 'flex', gap: 8 }}>
                                    ...
                                    <button
                                        key={est}
                                        onClick={() => setAsistencia(p => ({ ...p, [a.id]: est }))}
                                        style={{
                                        padding: '4px 12px',
                                        ...
                                        fontSize: '.78rem',
                                        fontWeight: 600,
                                        background: asistencia[a.id] === est
                                            ? est === 'presente' ? 'var(--success)' : est === 'ausente' ? 'var(--danger)' : 'var(--warning)'
                                            : 'transparent',
                                        color: asistencia[a.id] === est ? '#fff' : 'var(--muted)',
                                        borderColor: est === 'presente' ? 'var(--success)' : est === 'ausente' ? 'var(--danger)' : 'var(--warning)',
                                        }}
                                    >
        ...
        {tab === 'retiros' && (
            <div className="card">
            <div className="card-header">
                <div className="card-title">Registrar retiro de clase</div>
            </div>
            <form className="form-grid" onSubmit={handleCrearRetiro}>
                ...
                <select
                    ...
                    onChange={e => setRetiroForm(p => ({ ...p, alumnoId: e.target.value }))}
                    required
                >
                    <option value="">{alumnos.length ? 'Elegir alumno' : 'Selecciona un curso primero'}</option>
                    {alumnos.map(a => <option key={a.id} value={a.id}>{a.nombre}</option>)}
